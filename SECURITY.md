...
2. **GitHub**: Use [GitHub Security Advisories](https://github.com/Civigenesis/MultiClaw/security/advisories/new)
...
MultiClaw implements defense-in-depth security:
...
MultiClaw Docker images follow CIS Docker Benchmark best practices:
...
docker build -t multiclaw .
docker inspect --format='{{.Config.User}}' multiclaw
# Expected: 65534:65534

# Run with read-only filesystem (production hardening)
docker run --read-only -v /path/to/workspace:/workspace multiclaw gateway
```
