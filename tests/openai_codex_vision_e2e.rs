...
use anyhow::Result;
use multiclaw::providers::{ChatMessage, ChatRequest, Provider, ProviderRuntimeOptions};
...
async fn openai_codex_second_vision_support() -> Result<()> {
    ...
    let provider = multiclaw::providers::create_provider_with_options("openai-codex", None, &opts)?;
