...
<h1 align="center">MultiClaw 🦀（Русский）</h1>
...
|---|---|---|---|
| 2026-02-19 | _Срочно_ | Мы **не аффилированы** с `openagen/multiclaw` и `multiclaw.org`. Домен `multiclaw.org` сейчас указывает на fork `openagen/multiclaw`, и этот домен/репозиторий выдают себя за наш официальный сайт и проект. | Не доверяйте информации, бинарникам, сборам средств и «официальным» объявлениям из этих источников. Используйте только [этот репозиторий](https://github.com/Civigenesis/MultiClaw) и наши верифицированные соцсети. |
| 2026-02-21 | _Важно_ | Наш официальный сайт уже запущен: [multiclawlabs.ai](https://multiclawlabs.ai). Спасибо, что дождались запуска. При этом попытки выдавать себя за MultiClaw продолжаются, поэтому не участвуйте в инвестициях, сборах средств и похожих активностях, если они не подтверждены через наши официальные каналы. | Ориентируйтесь только на [этот репозиторий](https://github.com/Civigenesis/MultiClaw); также следите за [X (@multiclawlabs)](https://x.com/multiclawlabs?s=21), [Telegram (@multiclawlabs)](https://t.me/multiclawlabs), [Facebook (группа)](https://www.facebook.com/groups/multiclaw), [Reddit (r/multiclawlabs)](https://www.reddit.com/r/multiclawlabs/) и [Xiaohongshu](https://www.xiaohongshu.com/user/profile/67cbfc43000000000d008307?xsec_token=AB73VnYnGNx5y36EtnnZfGmAmS-6Wzv8WMuGpfwfkg6Yc%3D&xsec_source=pc_search) для официальных обновлений. |
...
MultiClaw — это производительная и расширяемая инфраструктура автономного AI-агента. MultiClaw — это **операционная система времени выполнения** для агентных рабочих процессов — инфраструктура, абстрагирующая модели, инструменты, память и выполнение, позволяя создавать агентов один раз и запускать где угодно.
...
## Снимок бенчмарка (MultiClaw vs OpenClaw, воспроизводимо)
...
> Примечание: результаты MultiClaw получены на release-сборке с помощью `/usr/bin/time -l`. OpenClaw требует Node.js runtime; только этот runtime обычно добавляет около 390MB дополнительного потребления памяти. NanoBot требует Python runtime. PicoClaw и MultiClaw — статические бинарники.

<p align="center">
  <img src="zero-claw.jpeg" alt="Сравнение MultiClaw и OpenClaw" width="800" />
</p>
...
- `multiclaw --help`: ~`0.02s`, пик памяти ~`3.9MB`
- `multiclaw status`: ~`0.01s`, пик памяти ~`4.1MB`
...
multiclaw onboard --api-key sk-... --provider openrouter
...
MultiClaw поддерживает нативные профили авторизации на основе подписки (мультиаккаунт, шифрование при хранении).

- Файл хранения: `~/.multiclaw/auth-profiles.json`
- Ключ шифрования: `~/.multiclaw/.secret_key`
...
<p align="center">
  <img src="docs/architecture.svg" alt="Архитектура MultiClaw" width="900" />
</p>

| Подсистема | Trait | Встроенные реализации | Расширение |
|-----------|-------|---------------------|------------|
| **AI-модели** | `Provider` | Каталог через `multiclaw providers` (сейчас 28 встроенных + алиасы, плюс пользовательские endpoint) | `custom:https://your-api.com` (OpenAI-совместимый) или `anthropic-custom:https://your-api.com` |
...
При указании неподдерживаемого `runtime.kind` MultiClaw завершается с явной ошибкой, а не молча откатывается к native.
