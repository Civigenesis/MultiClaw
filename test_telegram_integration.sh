#!/bin/bash
# MultiClaw Telegram Integration Test Suite
...
if [ -f "target/release/multiclaw" ]; then
    BINARY_SIZE=$(ls -lh target/release/multiclaw | awk '{print $5}')
    SIZE_BYTES=$(stat -f%z target/release/multiclaw 2>/dev/null || stat -c%s target/release/multiclaw)
...
CONFIG_PATH="$HOME/.multiclaw/config.toml"
if [ -f "$CONFIG_PATH" ]; then
    ...
    if grep -q "\[channels_config.telegram\]" "$CONFIG_PATH"; then
        ...
        warn "Telegram not configured - run 'multiclaw onboard' first"
    fi
else
    warn "No config file found - run 'multiclaw onboard' first"
fi
...
START_TIME=$(date +%s)
HEALTH_OUTPUT=$(timeout 10 target/release/multiclaw channel doctor 2>&1 || true)
END_TIME=$(date +%s)
