...
const SERVICE_LABEL: &str = "com.multiclaw.daemon";
const WINDOWS_TASK_NAME: &str = "MultiClaw Daemon";
...
fn start_linux(init_system: InitSystem) -> Result<()> {
    match init_system {
        InitSystem::Systemd => {
            ...
            run_checked(Command::new("systemctl").args(["--user", "start", "multiclaw.service"]))?;
        }
        InitSystem::Openrc => {
            run_checked(Command::new("rc-service").args(["multiclaw", "start"]))?;
        }
...
fn status_linux(config: &Config, init_system: InitSystem) -> Result<()> {
    match init_system {
        ...
        InitSystem::Openrc => {
            let out = run_capture(Command::new("rc-service").args(["multiclaw", "status"]))
                .unwrap_or_else(|_| "unknown".into());
            println!("Service state: {}", out.trim());
            println!("Unit: /etc/init.d/multiclaw");
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    Ok(())
...
fn uninstall_linux(config: &Config, init_system: InitSystem) -> Result<()> {
    match init_system {
        ...
        InitSystem::Openrc => {
            let init_script = Path::new("/etc/init.d/multiclaw");
            if init_script.exists() {
                if let Err(err) =
                    run_checked(Command::new("rc-update").args(["del", "multiclaw", "default"]))
                {
                    eprintln!(
                        "⚠️  Warning: Could not remove multiclaw from OpenRC default runlevel: {err}"
                    );
                }
                fs::remove_file(init_script)
                    .with_context(|| format!("Failed to remove {}", init_script.display()))?;
            }
            println!("✅ Service uninstalled (/etc/init.d/multiclaw)");
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    Ok(())
...
"#,
    ...
    println!("   Start with: multiclaw service start");
    Ok(())
}
...
fn install_linux_systemd(config: &Config) -> Result<()> {
    ...
    fs::write(&file, unit)?;
    let _ = run_checked(Command::new("systemctl").args(["--user", "daemon-reload"]));
    let _ = run_checked(Command::new("systemctl").args(["--user", "enable", "multiclaw.service"]));
...
/// Check if the multiclaw user exists and has expected properties.
...
fn check_multiclaw_user() -> Result<()> {
    ...
    let (del_cmd, add_cmd) = if is_alpine {
        (
            "deluser multiclaw && delgroup multiclaw",
            "addgroup -S multiclaw && adduser -S -s /sbin/nologin -H -D -G multiclaw multiclaw",
        )
    } else {
        ("userdel multiclaw", "useradd -r -s /sbin/nologin multiclaw")
    };
        ...
        Ok(output) if output.status.success() => {
            ...
            if parts.len() >= 7 {
                ...
                if uid.parse::<u32>().unwrap_or(999) >= 1000 {
                    bail!(
                        "User 'multiclaw' exists but has unexpected UID {} (expected system UID < 1000).\n\
                ...
                if !shell.contains("nologin") && !shell.contains("false") {
                    bail!(
                        "User 'multiclaw' exists but has unexpected shell '{}'.\n\
                ...
                if home != "/var/lib/multiclaw" && home != "/nonexistent" {
                    eprintln!(
                        "⚠️  Warning: multiclaw user has home directory '{}' (expected /var/lib/multiclaw or /nonexistent)",
                        home
                    );
                }
...
fn ensure_multiclaw_user() -> Result<()> {
    ...
    if is_alpine {
        let group_output = Command::new("getent").args(["group", "multiclaw"]).output();
        ...
        if !group_exists {
            let output = Command::new("addgroup")
                .args(["-S", "multiclaw"])
                .output()
                .context("Failed to create multiclaw group")?;

            if !output.status.success() {
                let stderr = String::from_utf8_lossy(&output.stderr);
                bail!("Failed to create multiclaw group: {}", stderr.trim());
            }
            println!("✅ Created system group: multiclaw");
        }
        ...
        let output = Command::new("adduser")
            ...
            .output()
            .context("Failed to create multiclaw user")?;

        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            bail!("Failed to create multiclaw user: {}", stderr.trim());
        }
    } else {
        let output = Command::new("useradd")
            .args(["-r", "-s", "/sbin/nologin", "multiclaw"])
            .output()
            .context("Failed to create multiclaw user")?;
    ...
    println!("✅ Created system user: multiclaw");
    Ok(())
}

/// Change ownership of a path to multiclaw:multiclaw
#[cfg(unix)]
fn chown_to_multiclaw(path: &Path) -> Result<()> {
    let output = Command::new("chown")
        .args(["multiclaw:multiclaw", &path.to_string_lossy()])
        .output()
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        bail!(
            "Failed to change ownership of {} to multiclaw:multiclaw: {}",
            path.display(),
            stderr.trim(),
        );
    }
...
#[cfg(not(unix))]
fn chown_to_multiclaw(_path: &Path) -> Result<()> {
    Ok(())
}

#[cfg(unix)]
fn chown_recursive_to_multiclaw(path: &Path) -> Result<()> {
    let output = Command::new("chown")
        .args(["-R", "multiclaw:multiclaw", &path.to_string_lossy()])
        .output()
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        bail!(
            "Failed to recursively change ownership of {} to multiclaw:multiclaw: {}",
            path.display(),
            stderr.trim(),
        );
    }
...
#[cfg(not(unix))]
fn chown_recursive_to_multiclaw(_path: &Path) -> Result<()> {
    Ok(())
}
    ...
    if let Some(user) = sudo_user {
        if let Ok(output) = Command::new("getent").args(["passwd", &user]).output() {
            if output.status.success() {
                ...
                if fields.len() >= 6 {
                    return Some(PathBuf::from(fields[5]).join(".multiclaw"));
                }
            }
        }
    }
...
fn ensure_openrc_runtime_path_writable(path: &Path) -> Result<()> {
    ...
    if !output.status.success() {
        ...
        bail!(
            "OpenRC runtime user 'multiclaw' cannot write {} ({details}). \
             Re-run `sudo multiclaw service install` and ensure ownership is multiclaw:multiclaw.",
            path.display(),
        );
    }
    ...
    if path_str.contains("/home/") || path_str.contains(".cargo/bin") {
        eprintln!(
            "⚠️  Warning: Binary path '{}' appears to be in a user home directory.\n\
             ...
             sudo cp '{}' /usr/local/bin/multiclaw",
            exe_path.display(),
            exe_path.display()
        );
    }
...
umask 027
output_log="/var/log/multiclaw/access.log"
error_log="/var/log/multiclaw/error.log"
...
fn resolve_openrc_executable() -> Result<PathBuf> {
    let preferred = Path::new("/usr/local/bin/multiclaw");
    if preferred.exists() {
...
fn install_linux_openrc(config: &Config) -> Result<()> {
    if !is_root() {
        bail!(
            "OpenRC service installation requires root privileges.\n\
             Please run with sudo: sudo multiclaw service install"
        );
    }

    ensure_multiclaw_user()?;

    let exe = resolve_openrc_executable()?;
    warn_if_binary_in_home(&exe);

    let config_dir = Path::new("/etc/multiclaw");
    let workspace_dir = config_dir.join("workspace");
    let log_dir = Path::new("/var/log/multiclaw");
    ...
    if !workspace_dir.exists() {
        ...
        chown_to_multiclaw(&workspace_dir)?;
        println!(
            "✅ Created directory: {} (owned by multiclaw:multiclaw)",
    ...
    chown_recursive_to_multiclaw(config_dir)?;
    ...
    run_checked(Command::new("rc-update").args(["add", "multiclaw", "default"]))?;
    println!("✅ Installed OpenRC service: /etc/init.d/multiclaw");
    println!("   Config path: /etc/multiclaw/config.toml");
    println!("   Start with: sudo multiclaw service start");
    let _ = config;
    Ok(())
}
...
fn install_windows(config: &Config) -> Result<()> {
    ...
    let wrapper = logs_dir.join("multiclaw-daemon.cmd");
...
mod tests {
    ...
    fn linux_service_file_has_expected_suffix() {
        ...
        assert!(path.ends_with(".config/systemd/user/multiclaw.service"));
    }

    #[test]
    fn windows_task_name_is_constant() {
        assert_eq!(windows_task_name(), "MultiClaw Daemon");
    }
    ...
    fn generate_openrc_script_contains_required_directives() {
        ...
        assert!(script.contains("description=\"MultiClaw daemon\""));
        assert!(script.contains("command=\"/usr/local/bin/multiclaw\""));
        assert!(script.contains("command_args=\"--config-dir /etc/multiclaw daemon\""));
        assert!(!script.contains("env MULTICLAW_CONFIG_DIR"));
        assert!(!script.contains("env MULTICLAW_WORKSPACE"));
        assert!(script.contains("command_background=\"yes\""));
        assert!(script.contains("command_user=\"multiclaw:multiclaw\""));
        assert!(script.contains("pidfile=\"/run/${RC_SVCNAME}.pid\""));
        assert!(script.contains("umask 027"));
        assert!(script.contains("output_log=\"/var/log/multiclaw/access.log\""));
        assert!(script.contains("error_log=\"/var/log/multiclaw/error.log\""));
    ...
    fn warn_if_binary_in_home_detects_home_path() {
        ...
        let home_path = PathBuf::from("/home/user/.cargo/bin/multiclaw");
        assert!(home_path.to_string_lossy().contains("/home/"));
        assert!(home_path.to_string_lossy().contains(".cargo/bin"));

        let cargo_path = PathBuf::from("/home/user/.cargo/bin/multiclaw");
        assert!(cargo_path.to_string_lossy().contains(".cargo/bin"));

        let system_path = PathBuf::from("/usr/local/bin/multiclaw");
    ...
    fn openrc_writability_probe_prefers_runuser_when_available() {
        let (program, args) =
            build_openrc_writability_probe_command(Path::new("/etc/multiclaw"), true);
        ...
        assert_eq!(
            ...
            vec![
                ...
                "sh".to_string(),
                "-c".to_string(),
                "test -w '/etc/multiclaw'".to_string()
            ]
        );
    }
    ...
    fn openrc_writability_probe_falls_back_to_su() {
        let (program, args) =
            build_openrc_writability_probe_command(Path::new("/etc/multiclaw/workspace"), false);
        ...
        assert_eq!(
            ...
            vec![
                ...
                "/bin/sh".to_string(),
                "-c".to_string(),
                "test -w '/etc/multiclaw/workspace'".to_string(),
                "multiclaw".to_string()
            ]
        );
    }
