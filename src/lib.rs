...
Examples:
  ...
  multiclaw channel add discord '{\"bot_token\":\"...\",\"name\":\"my-discord\"}'")]
    Add {
...
pub enum MigrateCommands {
    /// Import memory from an `OpenClaw` workspace into this `MultiClaw` workspace
    Openclaw {
...
Examples:
  ...
  multiclaw cron add-at 2025-12-31T23:59:00Z 'Happy New Year!'")]
    AddAt {
...
Examples:
  multiclaw cron add-every 60000 'Ping heartbeat'     # every minute
  multiclaw cron add-every 3600000 'Hourly report'    # every hour")]
    AddEvery {
...
Examples:
  ...
  multiclaw cron once 2h 'Follow up on deployment'
  multiclaw cron once 1d 'Daily check'")]
    Once {
        /// Delay duration
        delay: String,
        /// Command to run
...
Examples:
  multiclaw cron update <task-id> --expression '0 8 * * *'
  multiclaw cron update <task-id> --tz Europe/London --name 'Morning check'
  multiclaw cron update <task-id> --command 'Updated message'")]
    Update {
        /// Task ID
        id: String,
        /// New cron expression
...
Examples:
  ...
  multiclaw hardware introspect COM3")]
    Introspect {
...
Examples:
  ...
  multiclaw peripheral add esp32 /dev/ttyUSB0")]
    Add {
    ...
    /// Flash MultiClaw firmware to Arduino (creates .ino, installs arduino-cli if needed, uploads)
    #[command(long_about = "\
Flash MultiClaw firmware to an Arduino board.

Generates the .ino sketch, installs arduino-cli if it is not \
already available, compiles, and uploads the firmware.

Examples:
  multiclaw peripheral flash
  multiclaw peripheral flash --port /dev/cu.usbmodem12345
  multiclaw peripheral flash -p COM3")]
    Flash {
    ...
    /// Flash MultiClaw firmware to Nucleo-F401RE (builds + probe-rs run)
    FlashNucleo,
}
