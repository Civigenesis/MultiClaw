...
// in `~/.multiclaw/.secret_key` with restrictive file permissions (0600). The
...
pub struct SecretStore {
    /// Path to the key file (`~/.multiclaw/.secret_key`)
    key_path: PathBuf,
...
impl SecretStore {
    ...
    pub fn new(multiclaw_dir: &Path, enabled: bool) -> Self {
        Self {
            key_path: multiclaw_dir.join(".secret_key"),
            enabled,
        }
    }
