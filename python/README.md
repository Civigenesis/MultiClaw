...
Python companion package for [MultiClaw](https://github.com/Civigenesis/MultiClaw) — LangGraph-based tool calling for consistent LLM agent execution.
...
```bash
pip install multiclaw-tools[discord]
```
...
```python
import asyncio
from multiclaw_tools import create_agent, shell, file_read, file_write
...
# Run the CLI
multiclaw-tools "List files in the current directory"
...
```python
import os
from multiclaw_tools.integrations import DiscordBot
...
```python
from multiclaw_tools import tool
...
## Comparison with Rust MultiClaw

| Feature | Rust MultiClaw | multiclaw-tools |
|---------|---------------|----------------|
...
Use **Rust MultiClaw** for production edge deployments. Use **multiclaw-tools** when you need guaranteed tool calling consistency or Python ecosystem integration.
