...
Thanks for your interest in contributing to MultiClaw! This guide will help you get started.

## First-Time Contributors

Welcome — contributions of all sizes are valued. If this is your first contribution, here is how to get started:

1. **Find an issue.** Look for issues labeled [`good first issue`](https://github.com/Civigenesis/MultiClaw/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — these are scoped for newcomers and include context to get moving quickly.
...
4. **Start with Track A.** MultiClaw uses three [collaboration tracks](#collaboration-tracks-risk-based) (A/B/C) based on risk. First-time contributors should target **Track A** (docs, tests, chore) — these require lighter review and are the fastest path to a merged PR.
...
MultiClaw supports layered secret management for local development and CI hygiene.
...
2. **Config file** (`~/.multiclaw/config.toml`)
    - Persistent setup for long-term use
    - When `secrets.encrypt = true` (default), secret values are encrypted before save
    - Secret key is stored at `~/.multiclaw/.secret_key` with restricted permissions
    - Use `multiclaw onboard` for guided setup
...
3. Generic env vars (`MULTICLAW_API_KEY`, `API_KEY`)

Provider/model config overrides:

- `MULTICLAW_PROVIDER` / `PROVIDER`
- `MULTICLAW_MODEL`
...
- `~/.multiclaw/.secret_key` or similar key files
...
- If identity-like wording is required, use MultiClaw-centric labels only (for example: `MultiClawAgent`, `MultiClawOperator`, `multiclaw_user`).
...
- **Identity normalization**: when identity traits are unavoidable, use MultiClaw/project-native roles instead of personal or real-world identities.
...
MultiClaw's architecture is built on **traits** — every subsystem is swappable. This means contributing a new integration is as simple as implementing a trait and registering it in the factory function.
...
- **Identity-like labels**: if unavoidable, use MultiClaw-native identifiers only (`MultiClawAgent`, `multiclaw_user`, `multiclaw_node`).
...
- **Good identity-like label**: `MultiClawAgent`, `multiclaw_user`, `multiclaw_node`
...
- [ ] Any required identity-like wording uses MultiClaw/project-native labels only
