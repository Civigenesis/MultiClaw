<p align="center">
  <img src="multiclaw.png" alt="MultiClaw" width="200" />
</p>

<h1 align="center">MultiClaw 🦀</h1>
...
<p align="center">
  ...
  <a href="https://x.com/multiclawlabs?s=21"><img src="https://img.shields.io/badge/X-%40multiclawlabs-000000?style=flat&logo=x&logoColor=white" alt="X: @multiclawlabs" /></a>
  <a href="https://multiclawlabs.cn/group.jpg"><img src="https://img.shields.io/badge/WeChat-Group-B7D7A8?logo=wechat&logoColor=white" alt="WeChat Group" /></a>
  <a href="https://www.xiaohongshu.com/user/profile/67cbfc43000000000d008307?xsec_token=AB73VnYnGNx5y36EtnnZfGmAmS-6Wzv8WMuGpfwfkg6Yc%3D&xsec_source=pc_search"><img src="https://img.shields.io/badge/Xiaohongshu-Official-FF2442?style=flat" alt="Xiaohongshu: Official" /></a>
  <a href="https://t.me/multiclawlabs"><img src="https://img.shields.io/badge/Telegram-%40multiclawlabs-26A5E4?style=flat&logo=telegram&logoColor=white" alt="Telegram: @multiclawlabs" /></a>
  <a href="https://www.facebook.com/groups/multiclaw"><img src="https://img.shields.io/badge/Facebook-Group-1877F2?style=flat&logo=facebook&logoColor=white" alt="Facebook Group" /></a>
  <a href="https://www.reddit.com/r/multiclawlabs/"><img src="https://img.shields.io/badge/Reddit-r%2Fmulticlawlabs-FF4500?style=flat&logo=reddit&logoColor=white" alt="Reddit: r/multiclawlabs" /></a>
</p>
<p align="center">
...
<p align="center">
  MultiClaw is the <strong>runtime operating system</strong> for agentic workflows — infrastructure that abstracts models, tools, memory, and execution so agents can be built once and run anywhere.
</p>
...
| ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _Critical_  | We are **not affiliated** with `openagen/multiclaw`, `multiclaw.org` or `multiclaw.net`. The `multiclaw.org` and `multiclaw.net` domains currently points to the `openagen/multiclaw` fork, and that domain/repository are impersonating our official website/project.                                                                                       | Do not trust information, binaries, fundraising, or announcements from those sources. Use only [this repository](https://github.com/Civigenesis/MultiClaw) and our verified social accounts.                                                                                                                                                                                                                                                                                                                                                                                                                       |
| 2026-02-21 | _Important_ | Our official website is now live: [multiclawlabs.ai](https://multiclawlabs.ai). Thanks for your patience while we prepared the launch. We are still seeing impersonation attempts, so do **not** join any investment or fundraising activity claiming the MultiClaw name unless it is published through our official channels.                            | Use [this repository](https://github.com/Civigenesis/MultiClaw) as the single source of truth. Follow [X (@multiclawlabs)](https://x.com/multiclawlabs?s=21), [Telegram (@multiclawlabs)](https://t.me/multiclawlabs), [Facebook (Group)](https://www.facebook.com/groups/multiclaw), [Reddit (r/multiclawlabs)](https://www.reddit.com/r/multiclawlabs/), and [Xiaohongshu](https://www.xiaohongshu.com/user/profile/67cbfc43000000000d008307?xsec_token=AB73VnYnGNx5y36EtnnZfGmAmS-6Wzv8WMuGpfwfkg6Yc%3D&xsec_source=pc_search) for official updates. |
...
|                           | OpenClaw      | NanoBot        | PicoClaw        | MultiClaw 🦀          |
| ------------------------- | ------------- | -------------- | --------------- | -------------------- |
...
> Notes: MultiClaw results are measured on release builds using `/usr/bin/time -l`. OpenClaw requires Node.js runtime (typically ~390MB additional memory overhead), while NanoBot requires Python runtime. PicoClaw and MultiClaw are static binaries. The RAM figures above are runtime memory; build-time compilation requirements are higher.

<p align="center">
  <img src="zero-claw.jpeg" alt="MultiClaw vs OpenClaw Comparison" width="800" />
</p>
...
- `multiclaw --help`: about `0.02s` real time, ~`3.9MB` peak memory footprint
- `multiclaw status`: about `0.01s` real time, ~`4.1MB` peak memory footprint
...
Or skip the steps above and install everything (system deps, Rust, MultiClaw) in a single command:

```bash
curl -LsSf https://raw.githubusercontent.com/Civigenesis/MultiClaw/main/scripts/install.sh | bash
```
...
```bash
curl -fsSL https://raw.githubusercontent.com/Civigenesis/MultiClaw/main/scripts/bootstrap.sh | bash
```
...
<https://github.com/Civigenesis/MultiClaw/releases/latest>

Example (ARM64 Linux):

```bash
curl -fsSLO https://github.com/Civigenesis/MultiClaw/releases/latest/download/multiclaw-aarch64-unknown-linux-gnu.tar.gz
tar xzf multiclaw-aarch64-unknown-linux-gnu.tar.gz
install -m 0755 multiclaw "$HOME/.cargo/bin/multiclaw"
```
...
multiclaw gateway                # default: 127.0.0.1:42617
multiclaw gateway --port 0       # random port (security hardened)
...
multiclaw channel bind-telegram 123456789

# Get integration setup details
multiclaw integrations info Telegram
...
# On Alpine (OpenRC): sudo multiclaw service install

# Migrate memory from OpenClaw (safe preview first)
multiclaw migrate openclaw --dry-run
multiclaw migrate openclaw
```
...
MultiClaw now supports subscription-native auth profiles (multi-account, encrypted at rest).

- Store file: `~/.multiclaw/auth-profiles.json`
- Encryption key: `~/.multiclaw/.secret_key`
...
multiclaw auth login --provider openai-codex --device-code

# Browser/callback flow with paste fallback
multiclaw auth login --provider openai-codex --profile default
multiclaw auth paste-redirect --provider openai-codex --profile default

# Check / refresh / switch profile
multiclaw auth status
multiclaw auth refresh --provider openai-codex --profile default
multiclaw auth use --provider openai-codex --profile work
```
...
multiclaw auth paste-token --provider anthropic --profile default --auth-kind authorization

# Alias command
multiclaw auth setup-token --provider anthropic --profile default
```

Run the agent with subscription auth:

```bash
multiclaw agent --provider openai-codex -m "hello"
multiclaw agent --provider openai-codex --auth-profile openai-codex:work -m "hello"

# Anthropic supports both API key and auth token env vars:
# ANTHROPIC_AUTH_TOKEN, ANTHROPIC_OAUTH_TOKEN, ANTHROPIC_API_KEY
multiclaw agent --provider anthropic -m "hello"
```
...
<p align="center">
  <img src="docs/architecture.svg" alt="MultiClaw Architecture" width="900" />
</p>

| Subsystem         | Trait            | Ships with                                                                                                                                                                 | Extend                                                                                       |
| ----------------- | ---------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------- |
| **AI Models**     | `Provider`       | Provider catalog via `multiclaw providers` (built-ins + aliases, plus custom endpoints)                                                                                     | `custom:https://your-api.com` (OpenAI-compatible) or `anthropic-custom:https://your-api.com` |
...
When an unsupported `runtime.kind` is configured, MultiClaw now exits with a clear error instead of silently falling back to native.
...
# When provider = "postgres", MultiClaw uses PostgreSQL for memory persistence.
# The db_url key also accepts alias `dbURL` for backward compatibility.
#
# [storage.provider.config]
# provider = "postgres"
# db_url = "postgres://user:password@host:5432/multiclaw"
# schema = "public"
# table = "memories"
...
# MULTICLAW_LUCID_CMD=/usr/local/bin/lucid            # default: lucid
# MULTICLAW_LUCID_BUDGET=200                          # default: 200
# MULTICLAW_LUCID_LOCAL_HIT_THRESHOLD=3               # local hit count to skip external recall
# MULTICLAW_LUCID_RECALL_TIMEOUT_MS=120               # low-latency budget for lucid context recall
# MULTICLAW_LUCID_STORE_TIMEOUT_MS=800                # async sync timeout for lucid store
# MULTICLAW_LUCID_FAILURE_COOLDOWN_MS=15000           # cooldown after lucid failure to avoid repeated slow attempts
```

## Security

MultiClaw enforces security at **every layer** — not just the sandbox. It passes all items from the community security checklist.
...
> **Run your own nmap:** `nmap -p 1-65535 <your-host>` — MultiClaw binds to localhost only, so nothing is exposed unless you explicitly configure a tunnel.
...
For non-text replies, MultiClaw can send Telegram attachments when the assistant includes markers:
...
MultiClaw supports two WhatsApp backends:
...
2. **Configure MultiClaw:**

    ```toml
    [channels_config.whatsapp]
    session_path = "~/.multiclaw/state/whatsapp-web/session.db"
...
3. **Start channels/daemon and link device:**
    - Run `multiclaw channel start` (or `multiclaw daemon`).
...
6. **Test:** Send a message to your WhatsApp Business number — MultiClaw will respond via the LLM.
...
#   "metadata": {"session_name": "...", "source": "multiclaw.browser", "version": "..."}
# }
...
MultiClaw uses one provider key (`ollama`) for both local and remote Ollama deployments:
...
MultiClaw now supports `llama-server` as a first-class local provider:
...
MultiClaw supports [vLLM](https://docs.vllm.ai/) as a first-class local provider:
...
MultiClaw supports [Osaurus](https://github.com/dinoki-ai/osaurus) as a first-class local provider — a unified AI edge runtime for macOS that combines local MLX inference with cloud provider proxying and MCP support through a single endpoint:
...
## Python Companion Package (`multiclaw-tools`)

For LLM providers with inconsistent native tool calling (e.g., GLM-5/Zhipu), MultiClaw ships a Python companion package with **LangGraph-based tool calling** for guaranteed consistency:

```bash
pip install multiclaw-tools
```

```python
from multiclaw_tools import create_agent, shell, file_read
...
MultiClaw supports **identity-agnostic** AI personas through two formats:
...
[AIEOS](https://aieos.org) is a standardization framework for portable AI identity. MultiClaw supports AIEOS v1.1 JSON payloads, allowing you to:
...
MultiClaw accepts both canonical AIEOS generator payloads and compact legacy payloads, then normalizes them into one system prompt format.
...
MultiClaw supports two init systems for background services:

| Init System                    | Scope       | Config Path                 | Requires  |
| ------------------------------ | ----------- | --------------------------- | --------- |
| **systemd** (default on Linux) | User-level  | `~/.multiclaw/config.toml`   | No sudo   |
| **OpenRC** (Alpine)            | System-wide | `/etc/multiclaw/config.toml` | sudo/root |
...
sudo multiclaw service install
sudo rc-update add multiclaw default
sudo rc-service multiclaw start
```
...
You can also override at runtime with `MULTICLAW_OPEN_SKILLS_ENABLED`, `MULTICLAW_OPEN_SKILLS_DIR`, and `MULTICLAW_SKILLS_PROMPT_MODE` (`full` or `compact`).

Skill installs are now gated by a built-in static security audit. `multiclaw skills install <source>` blocks symlinks, script-like files, unsafe markdown link patterns, and high-risk shell payload snippets before accepting a skill. You can run `multiclaw skills audit <source_or_name>` to validate a local directory or an installed skill manually.
...
MultiClaw is configured to use `rustls` for HTTP/TLS dependencies; `--locked` keeps the transitive graph deterministic on fresh environments.
...
If MultiClaw helps your work and you want to support ongoing development, you can donate here:
...
**This is the only official MultiClaw repository:**

> https://github.com/Civigenesis/MultiClaw

Any other repository, organization, domain, or package claiming to be "MultiClaw" or implying affiliation with MultiClaw Labs is **unauthorized and not affiliated with this project**. Known unauthorized forks will be listed in [TRADEMARK.md](TRADEMARK.md).

If you encounter impersonation or trademark misuse, please [open an issue](https://github.com/Civigenesis/MultiClaw/issues).

---

## License

MultiClaw is dual-licensed for maximum openness and contributor protection:
...
The **MultiClaw** name and logo are trademarks of MultiClaw Labs. This license does not grant permission to use them to imply endorsement or affiliation. See [TRADEMARK.md](TRADEMARK.md) for permitted and prohibited uses.
...
New to MultiClaw? Look for issues labeled [`good first issue`](https://github.com/Civigenesis/MultiClaw/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — see our [Contributing Guide](CONTRIBUTING.md#first-time-contributors) for how to get started.
...
- New `Skill` → `~/.multiclaw/workspace/skills/<name>/`

---

**MultiClaw** — Zero overhead. Zero compromise. Deploy anywhere. Swap anything. 🦀

## Star History

<p align="center">
  <a href="https://www.star-history.com/#Civigenesis/MultiClaw&type=date&legend=top-left">
    <picture>
     <source media="(prefers-color-scheme: dark)" srcset="https://api.star-history.com/svg?repos=Civigenesis/MultiClaw&type=date&theme=dark&legend=top-left" />
     <source media="(prefers-color-scheme: light)" srcset="https://api.star-history.com/svg?repos=Civigenesis/MultiClaw&type=date&legend=top-left" />
     <img alt="Star History Chart" src="https://api.star-history.com/svg?repos=Civigenesis/MultiClaw&type=date&legend=top-left" />
    </picture>
  </a>
</p>
