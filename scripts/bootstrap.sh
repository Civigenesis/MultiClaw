...
usage() {
  cat <<'USAGE'
MultiClaw installer bootstrap engine

Usage:
  ./multiclaw_install.sh [options]
  ./bootstrap.sh [options]         # compatibility entrypoint

Modes:
  Default mode installs/builds MultiClaw only (requires existing Rust toolchain).
...
Examples:
  ./multiclaw_install.sh
  ./multiclaw_install.sh --guided
  ./multiclaw_install.sh --install-system-deps --install-rust
  ./multiclaw_install.sh --prefer-prebuilt
  ./multiclaw_install.sh --prebuilt-only
  ./multiclaw_install.sh --onboard --api-key "sk-..." --provider openrouter [--model "openrouter/auto"]
  ./multiclaw_install.sh --interactive-onboard
...
Environment:
  MULTICLAW_CONTAINER_CLI     Container CLI command (default: docker; auto-fallback: podman)
  MULTICLAW_DOCKER_DATA_DIR   Host path for Docker config/workspace persistence
  MULTICLAW_DOCKER_IMAGE      Docker image tag to build/run (default: multiclaw-bootstrap:local)
  MULTICLAW_API_KEY           Used when --api-key is not provided
  MULTICLAW_PROVIDER          Used when --provider is not provided (default: openrouter)
  MULTICLAW_MODEL             Used when --model is not provided
  MULTICLAW_BOOTSTRAP_MIN_RAM_MB   Minimum RAM threshold for source build preflight (default: 2048)
  MULTICLAW_BOOTSTRAP_MIN_DISK_MB  Minimum free disk threshold for source build preflight (default: 6144)
  MULTICLAW_DISABLE_ALPINE_AUTO_DEPS
...
should_attempt_prebuilt_for_resources() {
  ...
  min_ram_mb="${MULTICLAW_BOOTSTRAP_MIN_RAM_MB:-2048}"
  min_disk_mb="${MULTICLAW_BOOTSTRAP_MIN_DISK_MB:-6144}"
...
install_prebuilt_binary() {
  ...
  archive_url="https://github.com/Civigenesis/MultiClaw/releases/latest/download/multiclaw-${target}.tar.gz"
  temp_dir="$(mktemp -d -t multiclaw-prebuilt-XXXXXX)"
  archive_path="$temp_dir/multiclaw-${target}.tar.gz"
  ...
  extracted_bin="$temp_dir/multiclaw"
  if [[ ! -x "$extracted_bin" ]]; then
    extracted_bin="$(find "$temp_dir" -maxdepth 2 -type f -name multiclaw -perm -u+x | head -n 1 || true)"
  fi
  if [[ -z "$extracted_bin" || ! -x "$extracted_bin" ]]; then
    warn "Archive did not contain an executable multiclaw binary."
    rm -rf "$temp_dir"
    return 1
  fi
  ...
  mkdir -p "$install_dir"
  install -m 0755 "$extracted_bin" "$install_dir/multiclaw"
  rm -rf "$temp_dir"

  info "Installed pre-built binary to $install_dir/multiclaw"
...
run_pacman() {
  ...
  local pacman_rc=0
  pacman_cfg_tmp="$(mktemp /tmp/multiclaw-pacman.XXXXXX.conf)"
...
run_guided_installer() {
  ...
  echo
  echo "MultiClaw guided installer"
  ...
  if prompt_yes_no "Install multiclaw into cargo bin now?" "yes"; then
    SKIP_INSTALL=false
  else
    SKIP_INSTALL=true
  fi
...
resolve_container_cli() {
  local requested_cli
  requested_cli="${MULTICLAW_CONTAINER_CLI:-docker}"
  ...
  if [[ "$requested_cli" != "docker" ]]; then
    error "Set MULTICLAW_CONTAINER_CLI to an installed Docker-compatible CLI (e.g., docker or podman)."
  else
    error "Install Docker, install podman, or set MULTICLAW_CONTAINER_CLI to an available Docker-compatible CLI."
  fi
  exit 1
}
...
run_docker_bootstrap() {
  ...
  docker_image="${MULTICLAW_DOCKER_IMAGE:-multiclaw-bootstrap:local}"
  fallback_image="ghcr.io/Civigenesis/MultiClaw:latest"
  if [[ "$TEMP_CLONE" == true ]]; then
    default_data_dir="$HOME/.multiclaw-docker"
  else
    default_data_dir="$WORK_DIR/.multiclaw-docker"
  fi
  docker_data_dir="${MULTICLAW_DOCKER_DATA_DIR:-$default_data_dir}"
  DOCKER_DATA_DIR="$docker_data_dir"

  mkdir -p "$docker_data_dir/.multiclaw" "$docker_data_dir/workspace"
  ...
  if [[ "$SKIP_BUILD" == false ]]; then
    ...
    if ! "$CONTAINER_CLI" image inspect "$docker_image" >/dev/null 2>&1; then
      ...
      info "Pulling official MultiClaw image ($fallback_image)"
  ...
  config_mount="$docker_data_dir/.multiclaw:/multiclaw-data/.multiclaw"
  workspace_mount="$docker_data_dir/workspace:/multiclaw-data/workspace"
...
Use either:
  --api-key "sk-..."
or:
  MULTICLAW_API_KEY="sk-..." ./multiclaw_install.sh --docker
or run interactive:
...
MSG
  ...
  "$CONTAINER_CLI" run --rm -it \
    ...
    -e HOME=/multiclaw-data \
    -e MULTICLAW_WORKSPACE=/multiclaw-data/workspace \
    -v "$config_mount" \
    -v "$workspace_mount" \
    "$docker_image" \
    "${onboard_cmd[@]}"
...
REPO_URL="https://github.com/Civigenesis/MultiClaw.git"
ORIGINAL_ARG_COUNT=$#
GUIDED_MODE="auto"
...
CONTAINER_CLI="${MULTICLAW_CONTAINER_CLI:-docker}"
API_KEY="${MULTICLAW_API_KEY:-}"
PROVIDER="${MULTICLAW_PROVIDER:-openrouter}"
MODEL="${MULTICLAW_MODEL:-}"
...
if [[ "$DOCKER_MODE" == true ]]; then
  ...
  if [[ "$OS_NAME" == "Linux" && -z "${MULTICLAW_DISABLE_ALPINE_AUTO_DEPS:-}" ]] && have_cmd apk; then
    find_missing_alpine_prereqs
    if [[ ${#ALPINE_MISSING_PKGS[@]} -gt 0 && "$INSTALL_SYSTEM_DEPS" == false ]]; then
      ...
      info "Auto-enabling system dependency installation (set MULTICLAW_DISABLE_ALPINE_AUTO_DEPS=1 to disable)."
      INSTALL_SYSTEM_DEPS=true
    fi
  fi
...
if [[ ! -f "$WORK_DIR/Cargo.toml" ]]; then
  if [[ -f "$(pwd)/Cargo.toml" ]]; then
    ...
    TEMP_DIR="$(mktemp -d -t multiclaw-bootstrap-XXXXXX)"
...
Your containerized MultiClaw data is persisted under:
DONE
...
or re-run with:
  ./multiclaw_install.sh --install-rust
MSG
  exit 1
fi
...
if [[ "$SKIP_INSTALL" == false ]]; then
  info "Installing multiclaw to cargo bin"
...
MULTICLAW_BIN=""
if have_cmd multiclaw; then
  MULTICLAW_BIN="multiclaw"
elif [[ -x "$HOME/.cargo/bin/multiclaw" ]]; then
  MULTICLAW_BIN="$HOME/.cargo/bin/multiclaw"
elif [[ -x "$WORK_DIR/target/release/multiclaw" ]]; then
  MULTICLAW_BIN="$WORK_DIR/target/release/multiclaw"
fi

if [[ "$RUN_ONBOARD" == true ]]; then
  if [[ -z "$MULTICLAW_BIN" ]]; then
    error "onboarding requested but multiclaw binary is not available."
    error "Run without --skip-install, or ensure multiclaw is in PATH."
    exit 1
  fi

  if [[ "$INTERACTIVE_ONBOARD" == true ]]; then
    info "Running interactive onboarding"
    "$MULTICLAW_BIN" onboard --interactive
  else
    if [[ -z "$API_KEY" ]]; then
      cat <<'MSG'
==> Onboarding requested, but API key not provided.
Use either:
  --api-key "sk-..."
or:
  MULTICLAW_API_KEY="sk-..." ./multiclaw_install.sh --onboard
or run interactive:
  ./multiclaw_install.sh --interactive-onboard
MSG
      exit 1
    ...
    ONBOARD_CMD=("$MULTICLAW_BIN" onboard --api-key "$API_KEY" --provider "$PROVIDER")
    if [[ -n "$MODEL" ]]; then
      ONBOARD_CMD+=(--model "$MODEL")
    fi
    "${ONBOARD_CMD[@]}"
