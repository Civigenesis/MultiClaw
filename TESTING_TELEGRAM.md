...
This guide covers testing the Telegram channel integration for MultiClaw.
...
6. **Error logging**
    ...
    ```bash
    RUST_LOG=debug multiclaw channel start
    ```
...
Solution: Check user allowlist
  ...
  4. Run: multiclaw onboard --channels-only
```
...
# 3. Monitor logs
RUST_LOG=info multiclaw daemon
...
| ---------------------- | ---------- | -------------------------------- |
| Health check time      | <5s        | `time multiclaw channel doctor`   |
| First response time    | <3s        | Time from sending to receiving   |
| Message split overhead | <50ms      | Check logs for timing            |
| Memory usage           | <10MB      | `ps aux \| grep multiclaw`        |
| Binary size            | ~3-4MB     | `ls -lh target/release/multiclaw` |
...
```bash
# Maximum logging
RUST_LOG=trace multiclaw channel start
...
# Validate config
cat ~/.multiclaw/config.toml | grep -A 3 "\[channels_config.telegram\]"
```
...
- [MultiClaw Main README](README.md)
- [Contributing Guide](CONTRIBUTING.md)
- [Issue Tracker](https://github.com/Civigenesis/MultiClaw/issues)
