# AGENTS.md — MultiClaw Agent Engineering Protocol
...
MultiClaw is a Rust-first autonomous agent runtime optimized for:
...
- If identity-like naming is required in tests/examples, use MultiClaw-native labels only (`MultiClawAgent`, `multiclaw_user`, `multiclaw_node`).
...
- If identity-like context is unavoidable, use MultiClaw-scoped roles/labels only (for example: `MultiClawAgent`, `MultiClawOperator`, `multiclaw_user`) and avoid real-world personas.
- Recommended identity-safe naming palette (use when identity-like context is required):
    - actor labels: `MultiClawAgent`, `MultiClawOperator`, `MultiClawMaintainer`, `multiclaw_user`
    - service/runtime labels: `multiclaw_bot`, `multiclaw_service`, `multiclaw_runtime`, `multiclaw_node`
    - environment labels: `multiclaw_project`, `multiclaw_workspace`, `multiclaw_channel`
