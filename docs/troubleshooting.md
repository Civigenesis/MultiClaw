...
- `cargo check` / `cargo build` appears stuck at `Checking multiclaw` for a long time
- repeated `Blocking waiting for file lock on package cache` or `build directory`

Why this happens in MultiClaw:
...
### `multiclaw` command not found after install

Symptom:

- install succeeds but shell cannot find `multiclaw`
...
- stop extra `multiclaw daemon` / `multiclaw channel start` processes
