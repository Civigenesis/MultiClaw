...
This document is the canonical reference for channel configuration in MultiClaw.
...
6. **Stale runtime state**: config changed but `multiclaw daemon` was not restarted.

---

## 1. Configuration Namespace

All channel settings live under `channels_config` in `~/.multiclaw/config.toml`.
...
When running `multiclaw channel start` (or daemon mode), Telegram and Discord now support sender-scoped runtime switching:
...
- Model cache previews come from `multiclaw models refresh --provider <ID>`.
- These are runtime chat commands, not CLI subcommands.

## Inbound Image Marker Protocol

MultiClaw supports multimodal input through inline message markers:
...
If `[channels_config.matrix]`, `[channels_config.lark]`, or `[channels_config.feishu]` is present but the corresponding feature is not compiled in, `multiclaw channel list`, `multiclaw channel doctor`, and `multiclaw channel start` will report that the channel is intentionally skipped for this build.
...
access_token = "syt_..."
user_id = "@multiclaw:matrix.example.com"   # optional, recommended for E2EE
...
port = 6697
nickname = "multiclaw-bot"
username = "multiclaw"              # optional
channels = ["#multiclaw"]
allowed_users = ["*"]
...
- `MULTICLAW_NEXTCLOUD_TALK_WEBHOOK_SECRET` overrides config secret.
...
- `MULTICLAW_LINQ_SIGNING_SECRET` overrides config secret.
...
5. Restart `multiclaw daemon` after config changes.
...
```bash
RUST_LOG=info multiclaw daemon 2>&1 | tee /tmp/multiclaw.log
```

Then filter channel/gateway events:

```bash
rg -n "Matrix|Telegram|Discord|Slack|Mattermost|Signal|WhatsApp|Email|IRC|Lark|DingTalk|QQ|iMessage|Nostr|Webhook|Channel" /tmp/multiclaw.log
```
