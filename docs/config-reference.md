# MultiClaw Config Reference (Operator-Oriented)
...
1. `MULTICLAW_WORKSPACE` override (if set)
2. persisted `~/.multiclaw/active_workspace.toml` marker (if present)
3. default `~/.multiclaw/config.toml`

MultiClaw logs the resolved config on startup at `INFO` level:

- `Config loaded` with fields: `path`, `workspace`, `source`, `initialized`

Schema export command:

- `multiclaw config schema` (prints JSON Schema draft 2020-12 to stdout)
...
| `otel_service_name` | `multiclaw` | Service name emitted to OTLP collector |
...
- Query runtime traces with:
  - `multiclaw doctor traces --limit 20`
  - `multiclaw doctor traces --event tool_call_result --contains \"error\"`
  - `multiclaw doctor traces --id <trace-id>`
...
1. `MULTICLAW_PROVIDER` (explicit override, always wins when non-empty)
...
- Use `MULTICLAW_PROVIDER` when you intentionally want runtime env to override a non-default configured provider.
...
- When `enabled = true` and no OTP secret exists, MultiClaw generates one and prints an enrollment URI once.
...
| `state_file` | `~/.multiclaw/estop-state.json` | Persistent estop state path |
...
- Use CLI command `multiclaw estop` to engage and `multiclaw estop resume` to clear levels.
...
- Security-first default: MultiClaw does **not** clone or sync `open-skills` unless `open_skills_enabled = true`.
- Environment overrides:
  - `MULTICLAW_OPEN_SKILLS_ENABLED` accepts `1/0`, `true/false`, `yes/no`, `on/off`.
  - `MULTICLAW_OPEN_SKILLS_DIR` overrides the repository path when non-empty.
  - `MULTICLAW_SKILLS_PROMPT_MODE` accepts `full` or `compact`.
- Precedence for enable flag: `MULTICLAW_OPEN_SKILLS_ENABLED` → `skills.open_skills_enabled` in `config.toml` → default `false`.
...
- Skill loading and `multiclaw skills install` both apply a static security audit. Skills that contain symlinks, script-like files, high-risk shell payload snippets, or unsafe markdown link traversal are rejected.
...
- MultiClaw requests Composio v3 tools with `toolkit_versions=latest` and executes tools with `version="latest"` to avoid stale default tool revisions.
...
3. Validate with `multiclaw doctor` before restart/rollout.
...
- While `multiclaw channel start` is running, updates to `default_provider`, `default_model`, `default_temperature`, `api_key`, `api_url`, and `reliability.*` are hot-applied from `config.toml` on the next inbound message.
...
- `MULTICLAW_LINQ_SIGNING_SECRET` overrides `signing_secret` when set.
...
- `MULTICLAW_NEXTCLOUD_TALK_WEBHOOK_SECRET` overrides `webhook_secret` when set.
