...
This page defines the fastest supported path to install and initialize MultiClaw.
...
Default behavior is **app-only** (build/install MultiClaw) and expects existing Rust toolchain.
...
```bash
curl -fsSL https://raw.githubusercontent.com/Civigenesis/MultiClaw/main/scripts/install.sh | bash
```
...
This builds a local MultiClaw image and launches onboarding inside a container while
persisting config/workspace to `./.multiclaw-docker`.

Container CLI defaults to `docker`. If Docker CLI is unavailable and `podman` exists,
bootstrap auto-falls back to `podman`. You can also set `MULTICLAW_CONTAINER_CLI`
explicitly (for example: `MULTICLAW_CONTAINER_CLI=podman ./bootstrap.sh --docker`).
...
Docker tag (`MULTICLAW_DOCKER_IMAGE`, default: `multiclaw-bootstrap:local`); if missing,
it pulls `ghcr.io/Civigenesis/MultiClaw:latest` and tags it locally before running.
...
```bash
MULTICLAW_API_KEY="sk-..." MULTICLAW_PROVIDER="openrouter" ./bootstrap.sh --onboard
```
...
- `--install-system-deps`
- `--install-rust`
- `--skip-build` (in `--docker` mode: use local image if present, otherwise pull `ghcr.io/Civigenesis/MultiClaw:latest`)
- `--skip-install`
- `--provider <id>`
