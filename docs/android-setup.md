...
MultiClaw provides prebuilt binaries for Android devices.
...
The easiest way to run MultiClaw on Android is via [Termux](https://termux.dev/).
...
# For 64-bit (aarch64):
curl -LO https://github.com/Civigenesis/MultiClaw/releases/latest/download/multiclaw-aarch64-linux-android.tar.gz
tar xzf multiclaw-aarch64-linux-android.tar.gz

# For 32-bit (armv7):
curl -LO https://github.com/Civigenesis/MultiClaw/releases/latest/download/multiclaw-armv7-linux-androideabi.tar.gz
tar xzf multiclaw-armv7-linux-androideabi.tar.gz
```
...
For advanced users who want to run MultiClaw outside Termux:

```bash
# From your computer with ADB
adb push multiclaw /data/local/tmp/
adb shell chmod +x /data/local/tmp/multiclaw
adb shell /data/local/tmp/multiclaw --version
```
