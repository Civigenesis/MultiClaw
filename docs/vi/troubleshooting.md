...
- `cargo check` / `cargo build` dừng lâu ở `Checking multiclaw`
...
### Không tìm thấy lệnh `multiclaw` sau cài đặt

Triệu chứng:

- Cài đặt thành công nhưng shell không tìm thấy `multiclaw`
...
Xác minh `~/.multiclaw/config.toml`:
...
- Dừng các tiến trình `multiclaw daemon` / `multiclaw channel start` thừa
