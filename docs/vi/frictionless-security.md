...
[INFO] ✓ Audit logging enabled (~/.config/multiclaw/audit.log)
...
$ multiclaw security --status
Security Status:
  ✓ Sandbox: Landlock (Linux kernel 6.2)
  ✓ Memory monitoring: 512MB limit
  ✓ Audit logging: ~/.config/multiclaw/audit.log
  → 47 events logged today

# Tắt sandbox tường minh (ghi vào config)
$ multiclaw config set security.sandbox.enabled false

# Bật backend cụ thể
$ multiclaw config set security.sandbox.backend firejail

# Điều chỉnh giới hạn
$ multiclaw config set security.resources.max_memory_mb 2048
```
...
```bash
$ multiclaw onboard --security-level paranoid
[1/9] Workspace Setup...
...
...
✅ **Hiển thị trạng thái** — `multiclaw security --status`
