...
## Vấn đề
MultiClaw ghi log các hành động nhưng thiếu audit trail chống giả mạo cho:
...
# Chống giả mạo
sign_events = true
signing_key_path = "~/.config/multiclaw/audit.key"
...
multiclaw audit --user @alice

# Hiển thị tất cả lệnh rủi ro cao
multiclaw audit --risk high

# Hiển thị vi phạm trong 24 giờ qua
multiclaw audit --since 24h --violations-only

# Xuất sang JSON để phân tích
multiclaw audit --format json --output audit.json
