...
- Keep merge-blocking rust quality policy aligned across `.github/workflows/ci-run.yml`, `dev/ci.sh`, and `.githooks/pre-push` (`./scripts/ci/rust_quality_gate.sh` + `./scripts/ci/rust_strict_delta_gate.sh`). Lint/test/release `cargo` steps assume a fresh `web/dist/` from `./scripts/ci/web_build_gate.sh` (invoked by `dev/ci.sh` and GitHub Actions before compiling).
