...
MultiClaw supports custom API endpoints for both OpenAI-compatible and Anthropic-compatible providers.
...
# or: export MULTICLAW_API_KEY="your-api-key"
multiclaw agent
```

## llama.cpp Server (Recommended Local Setup)

MultiClaw includes a first-class local provider for `llama-server`:
...
```bash
multiclaw models refresh --provider llamacpp
multiclaw agent -m "hello"
```

You do not need to export `MULTICLAW_API_KEY=dummy` for this flow.

## SGLang Server

MultiClaw includes a first-class local provider for [SGLang](https://github.com/sgl-project/sglang):
...
```bash
multiclaw models refresh --provider sglang
multiclaw agent -m "hello"
```
...
MultiClaw includes a first-class local provider for [vLLM](https://docs.vllm.ai/):
...
```bash
multiclaw models refresh --provider vllm
multiclaw agent -m "hello"
```
