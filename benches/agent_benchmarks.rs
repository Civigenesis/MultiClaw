//! Performance benchmarks for MultiClaw hot paths.
//!
//! Benchmarks cover:
...
//! Run: `cargo bench`
//!
//! Ref: https://github.com/Civigenesis/MultiClaw/issues/618 (item 7)

use criterion::{criterion_group, criterion_main, Criterion};
use std::hint::black_box;
use std::sync::{Arc, Mutex};

use multiclaw::agent::agent::Agent;
use multiclaw::agent::dispatcher::{NativeToolDispatcher, ToolDispatcher, XmlToolDispatcher};
use multiclaw::config::MemoryConfig;
use multiclaw::memory;
use multiclaw::memory::{Memory, MemoryCategory};
use multiclaw::observability::{NoopObserver, Observer};
use multiclaw::providers::{ChatRequest, ChatResponse, Provider, ToolCall};
use multiclaw::tools::{Tool, ToolResult};
...
<tool_call>
{"name": "search", "arguments": {"query": "multiclaw architecture"}}
</tool_call>
    ...
    let response = ChatResponse {
        ...
        tool_calls: vec![
            ToolCall {
                id: "tc1".into(),
                name: "search".into(),
                arguments: r#"{"query": "multiclaw"}"#.into(),
            },
            ToolCall {
                id: "tc2".into(),
                name: "read_file".into(),
...
fn bench_memory_operations(c: &mut Criterion) {
    ...
    rt.block_on(async {
        for i in 0..100 {
            mem.store(
                &format!("key_{i}"),
                &format!("Content entry number {i} about multiclaw agent runtime"),
                MemoryCategory::Core,
                None,
            )
            .await
    ...
    c.bench_function("memory_recall_top10", |b| {
        b.iter(|| {
            rt.block_on(async {
                mem.recall(black_box("multiclaw agent"), 10, None)
                    .await
                    .unwrap()
            })
        });
