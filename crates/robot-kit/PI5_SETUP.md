...
Complete guide to setting up a MultiClaw-powered robot on Raspberry Pi 5.
...
mkdir -p ~/.multiclaw/models
cp models/ggml-base.bin ~/.multiclaw/models/
```
...
mkdir -p ~/.multiclaw/models/piper
cd ~/.multiclaw/models/piper
wget https://huggingface.co/rhasspy/piper-voices/resolve/main/en/en_US/lessac/medium/en_US-lessac-medium.onnx
wget https://huggingface.co/rhasspy/piper-voices/resolve/main/en/en_US/lessac/medium/en_US-lessac-medium.onnx.json

# Test
echo "Hello, I am your robot!" | piper --model ~/.multiclaw/models/piper/en_US-lessac-medium.onnx --output_file test.wav
aplay test.wav
```
...
### 7. Build MultiClaw Robot Kit

```bash
# Clone repo (or copy from USB)
git clone https://github.com/Civigenesis/MultiClaw
cd multiclaw

# Build robot kit
cargo build --release -p multiclaw-robot-kit

# Build main multiclaw (optional, if using as agent)
cargo build --release
```
...
```bash
mkdir -p ~/.multiclaw
nano ~/.multiclaw/robot.toml
```

```toml
# ~/.multiclaw/robot.toml - Real Hardware Configuration
...
# Test speaker
echo "Testing speaker" | piper --model ~/.multiclaw/models/piper/en_US-lessac-medium.onnx --output_file - | aplay -D plughw:0,0
...
FIFO_PATH = "/tmp/multiclaw_sensors.fifo"
...
### Start MultiClaw Agent

```bash
# Configure MultiClaw to use robot tools
cat > ~/.multiclaw/config.toml << 'EOF'
...
cp ~/multiclaw/crates/robot-kit/SOUL.md ~/.multiclaw/workspace/

# Start agent
./target/release/multiclaw agent
```
...
# Start sensor loop
if [ ! -p /tmp/multiclaw_sensors.fifo ]; then
    mkfifo /tmp/multiclaw_sensors.fifo
fi
...
# Start multiclaw
cd ~/multiclaw
./target/release/multiclaw daemon &
AGENT_PID=$!
...
```bash
# /etc/systemd/system/multiclaw-robot.service
sudo tee /etc/systemd/system/multiclaw-robot.service << 'EOF'
[Unit]
Description=MultiClaw Robot
After=network.target ollama.service

[Service]
Type=simple
User=pi
WorkingDirectory=/home/pi/multiclaw
...
sudo systemctl enable multiclaw-robot
sudo systemctl start multiclaw-robot

# Check status
sudo systemctl status multiclaw-robot
journalctl -u multiclaw-robot -f  # View logs
```
