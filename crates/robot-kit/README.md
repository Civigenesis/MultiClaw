...
A complete toolkit for building AI-powered robots with MultiClaw. Designed for Raspberry Pi deployment with offline Ollama inference.
...
### 1. Build MultiClaw with robot tools

```bash
# Clone and build
git clone https://github.com/Civigenesis/MultiClaw
cd multiclaw
cargo build -p multiclaw-robot-kit --release
```
...
# Copy config
mkdir -p ~/.multiclaw
cp crates/robot-kit/robot.toml ~/.multiclaw/
cp crates/robot-kit/SOUL.md ~/.multiclaw/workspace/
...
# Test in mock mode
./target/release/multiclaw agent -m "Say hello and show a happy face"

# Test with real hardware
# (after configuring robot.toml)
./target/release/multiclaw agent -m "Move forward 1 meter"
```
...
```rust
use multiclaw_robot_kit::{create_tools, RobotConfig};
...
If you want runtime registration in `multiclaw`, add a thin adapter that maps this
...
mkdir multiclaw-robot-kit
cp -r target/release/multiclaw multiclaw-robot-kit/
cp -r examples/robot_kit multiclaw-robot-kit/
cp -r ~/.multiclaw multiclaw-robot-kit/dot-multiclaw

# Include models
mkdir -p multiclaw-robot-kit/models
cp ~/.multiclaw/models/ggml-base.bin multiclaw-robot-kit/models/
# Note: Ollama models are large, may want to download on target

# Create tarball
tar -czvf multiclaw-robot-kit.tar.gz multiclaw-robot-kit/

# Copy to USB
cp multiclaw-robot-kit.tar.gz /media/usb/TarBalls/
```
